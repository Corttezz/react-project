...
export default function FitnessLevel() {
  ...
  const updateLevelInDatabase = async () => {
    ...
    try {
      ...
      const response = await axios.put(`https://backend-server-inteligym.azurewebsites.net/updateLevel/${userId}`, {
        level: selectedLevel
      }, {
        headers: {
      ...
      if (response.status === 200) {
        ...
        console.error("Erro ao atualizar o nível.");
      }
    } catch (error) {
    ...
    <View style={styles.container}>
      ...
      <Animatable.View delay={200} animation="fadeInDown" style={styles.iconContainer}>
        <View style={styles.header}>
          <Text style={styles.title}>Qual é o seu nível de fitness?</Text>
          <Text style={styles.subtitle}>Selecione um nível que melhor descreva sua experiência em fitness!</Text>
        </View>
      </Animatable.View>

      <Animatable.View delay={200} animation="fadeInRight" style={styles.iconContainer}>
  <View style={styles.levelContainer}>
    {["Iniciante", "Intermediário", "Avançado"].map((level, index) => (
      <TouchableOpacity 
        key={index} 
